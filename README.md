...
出力フォルダ、ソース名、ターゲット名を入力します。出力フォルダは JSON の保存先であり、`avatar_data_ターゲット名.json` や `pose_basis_template.json` の検索先にもなります（未指定の場合は `.blend` ファイルの保存先フォルダが使われます）。ソース名・ターゲット名はそれぞれ `avatar_data_ターゲット名.json` の自動検索、および出力ファイル名の生成に使われます。
